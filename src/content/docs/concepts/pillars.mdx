...
- **Effort is plural.** A `WorkUnit` may carry an array of `EffortLoad` entries, each
  tagging its method (`RPE｜RIR｜sRPE｜TRIMP｜TSS｜VBT｜%1RM｜…`) and source.
...
- **Sets shorthand.** A *planned* `prescription` may use `sets: N`, defined to expand to N
  identical `WorkUnit`s. A *performed* record enumerates one `WorkUnit` per actual set.
