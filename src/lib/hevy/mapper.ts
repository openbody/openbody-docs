// VENDORED, browser-safe copy of openbody-ts's `src/mappers/hevy.ts` (Apache-2.0). See
// `./csv.ts` for why this is a copy rather than a direct import of `@openbody/openbody-ts`,
// and the TODO to remove it once that's fixed upstream. Kept intentionally close to the
// original so the two are easy to diff.
//
// Hevy CSV export -> OpenBody Session/Block/Exercise/WorkUnit records.
import { parseCsv, num, toRfc3339, type OpenBodyRecord, type MapOptions } from "./csv";

const SET_ROLE: Record<string, string> = {
  normal: "working",
  warmup: "warmup",
  drop: "drop",
  failure: "failure",
...
export function mapHevy(csv: string, opts: MapOptions = {}): OpenBodyRecord[] {
  ...
  for (const [, srows] of sessions) {
    ...
    const session: OpenBodyRecord = {
      ...
      startTime: toRfc3339(f.start_time),
      endTime: toRfc3339(f.end_time),
    };
    ...
    if (hasSuperset) {
      // At-most-one container: any superset -> everything goes under blocks[].
